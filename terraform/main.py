...
"""
    ...
    def create_pr_from_session_data(self, session_id: str, summary: str, session_data: Dict[str, Any],
                                     repo_label: Optional[str] = None, repo_name: Optional[str] = None) -> str:
        """Create a PR with session data and summary."""
        try:
            repo_config = self._get_repo_for_operation('sessions', repo_label, repo_name)
            repo = repo_config.get_repo()
            ...
            base_path = repo_config.paths.get('sessions', 'sessions/')
            date_str = datetime.utcnow().strftime('%Y-%m-%d')
            session_folder = f"{base_path}{date_str}-{session_id.split('@')[0]}"

            # Commit session_data.json
            repo.create_file(
                path=f"{session_folder}/session_data.json",
                message=f"Add session data for {session_id}",
            ...
            # Commit summary.md
            repo.create_file(
                path=f"{session_folder}/summary.md",
                message=f"Add session summary for {session_id}",
                content=summary,
                branch=branch_name
            )

            pr = repo.create_pull(
                title=f"Session Data and Summary: {session_id}",
                body=f"Automated PR containing session data and summary for session `{session_id}`.",
                head=branch_name,
                base=default_branch
            )
            ...
            logger.error(f"Error creating session data PR: {e}", exc_info=True)
            raise
...
class TelegramWebhookHandler:
    ...
    def handle(self, update_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            elif message_text.startswith('/set_session_data'):
                return self._handle_set_session_data_command(session_id, chat_id, message_text)

            elif message_text.startswith('/view_session_data'):
                return self._handle_view_session_data_command(session_id, chat_id)
    ...
    def _handle_set_session_data_command(self, session_id: str, chat_id: int, message_text: str) -> Dict[str, Any]:
        """Handle /set_session_data command."""
        try:
            parts = message_text.split(' ', 1)
            if len(parts) < 2:
                self.telegram_client.send_message(chat_id, 'Usage: /set_session_data <json_data>')
                return {'status': 'ok'}

            json_data_str = parts[1]
            try:
                data = json.loads(json_data_str)
                FirestoreManager.save_session_data(session_id, data)
                self.telegram_client.send_message(chat_id, '✅ Session data updated successfully.')
            except json.JSONDecodeError:
                self.telegram_client.send_message(chat_id, '❌ Invalid JSON format.')
        except Exception as e:
            logger.error(f"Error setting session data: {e}")
            self.telegram_client.safe_send_error(chat_id, e, context="setting session data")
        return {'status': 'ok'}

    def _handle_view_session_data_command(self, session_id: str, chat_id: int) -> Dict[str, Any]:
        """Handle /view_session_data command."""
        try:
            data = FirestoreManager.get_session_data(session_id)
            if data:
                pretty_json = json.dumps(data, indent=2)
                message = f"```json\n{pretty_json}\n```"
                self.telegram_client.send_message(chat_id, message, parse_mode='MarkdownV2')
            else:
                self.telegram_client.send_message(chat_id, 'No session data found.')
        except Exception as e:
            logger.error(f"Error viewing session data: {e}")
            self.telegram_client.safe_send_error(chat_id, e, context="viewing session data")
        return {'status': 'ok'}
    ...
    def _handle_done_request(self, session_id: str, chat_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Regular /done flow (approval workflow)
            history = FirestoreManager.get_session_history(session_id)
            session_data = FirestoreManager.get_session_data(session_id)
            ...
            # Generate summary
            summary_prompt = f"Please provide a concise summary of the following conversation, taking into account the provided session data:\n\nSession Data:\n```json\n{json.dumps(session_data, indent=2)}\n```\n\nConversation History:\n"
            for msg in history:
                summary_prompt += f"{msg['role']}: {msg['content']}\n"

            summary = self.gemini.chat_response(summary_prompt, [])

            # Create validation ID
            validation_id = f"session_summary_{session_id}"
            ...
            FirestoreManager.save_pending_validation(
                ...
                filename='summary.md',
                content=summary,
                structured_notes=json.dumps(session_data)
            )

            # Send summary for approval
            message = f"**Session Summary:**\n\n{summary}\n\n**Session Data:**\n```json\n{json.dumps(session_data, indent=2)}\n```\n\nPlease approve to save this session to the repository."
            keyboard = [
                [
                    InlineKeyboardButton("✅ Save to Repo", callback_data=f"approve_session:{validation_id}"),
                    InlineKeyboardButton("❌ Cancel", callback_data=f"reject_session:{validation_id}")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            self.telegram_client.send_message(chat_id, message, reply_markup=reply_markup)
            return {'status': 'ok'}
        except Exception as e:
...
PROCEED TO PHASE 3: FINALIZATION. Generate the complete structured notes in Obsidian-compatible markdown format with YAML frontmatter, following all requirements from the prompt."""
    ...
    def _handle_callback_query(self, callback_query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == 'validate':
                ...
                return self._handle_session_approve(query_id, chat_id, pending)
            elif action == 'reject_session':
                return self._handle_session_reject(query_id, chat_id, pending)
            else:
    ...
    def _handle_session_approve(self, query_id: str, chat_id: int, pending: Dict[str, Any]) -> Dict[str, Any]:
        """Handle session approval."""
        try:
            validation_id = pending['validation_id']
            session_id = pending['session_id']
            summary = pending['content']
            session_data = json.loads(pending['structured_notes'])

            self.telegram_client.answer_callback_query(query_id, "✅ Approving session...")
            self.telegram_client.send_message(chat_id, "🚀 Creating pull request...")

            pr_url = self.github_manager.create_pr_from_session_data(session_id, summary, session_data)

            message = f"✅ Session saved! Pull request created:\n{pr_url}"
            self.telegram_client.send_message(chat_id, message)
            ...
            self.telegram_client.safe_send_error(chat_id, e, context="approving session")
            return {'status': 'error'}

    def _handle_session_reject(self, query_id: str, chat_id: int, pending: Dict[str, Any]) -> Dict[str, Any]:
        """Handle session rejection."""
        try:
            validation_id = pending['validation_id']
            self.telegram_client.answer_callback_query(query_id, "❌ Session save canceled.")
            self.telegram_client.send_message(chat_id, "Session save has been canceled.")
            FirestoreManager.delete_pending_validation(validation_id)
            return {'status': 'ok'}
        except Exception as e:
            logger.error(f"Error rejecting session: {e}")
            self.telegram_client.safe_send_error(chat_id, e, context="rejecting session")
            return {'status': 'error'}
...
Continue the interview process. Ask follow-up clarifying questions if needed, or indicate if you have enough information to proceed to finalization."""
            ...
            next_response = self.gemini.chat_response(context_message, history, session_data)
    ...
    def _handle_chat_message(self, session_id: str, chat_id: int, username: str, message_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get history and session data
            history = FirestoreManager.get_session_history(session_id)
            session_data = FirestoreManager.get_session_data(session_id)

            # Generate response
            bot_response = self.gemini.chat_response(message_text, history, session_data)
