...
1. **Make Changes**: Modify files in `terraform/`
...
2. Update the `WORKING_DIR` to `terraform`
...
**Solution**: Update workflow to use `terraform/` instead of `./terraform`
...
**Solution**: Ensure changes are in `terraform/**` path and pushed to a non-main branch
