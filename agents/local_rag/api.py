...
async def query(request: QueryRequest):
    ...
    try:
        ...
        # Query the agent
        results = agent.query(request.query, top_k=request.top_k)
...
async def ingest(request: IngestRequest):
    ...
    try:
        ...
        if hasattr(agent, 'ingest_documents'):
            count = agent.ingest_documents(force_refresh=request.force_refresh)
        else:
            count = 0  # Agent already initialized
