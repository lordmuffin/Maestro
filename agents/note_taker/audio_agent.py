...
def generate_notes_node(state: InterviewerState) -> Dict[str, Any]:
    ...
    well-formatted notes ready for Obsidian with YAML frontmatter.

    Format follows the template defined in prompts/interviewer_prompt.md
    """
    ...
    # TODO: Replace with actual LLM generation using the system prompt
    # For now, create a structured format matching the prompt template
...
[Executive summary to be generated by LLM - This is a placeholder showing the first 200 characters of the transcript]

{transcript[:200]}...

## Key Decisions

- **Decision:** [To be extracted by LLM] - **Rationale:** [Why it was decided]

## Action Items

- [ ] [Specific action to be extracted by LLM] - **Owner:** [[TBD]] - **Due:** {date_str}
...
- [Additional details to be extracted by LLM]
"""

    # Add clarifications if any were provided
    if clarifications:
...
- [Follow-up actions to be determined by LLM]
- [Dependencies or future meetings]

## Notes

[Additional context and observations to be generated by LLM]
"""

    # Add raw transcript in collapsible section
    notes += f"""
## Raw Transcript
...
"""
    ...
    logger.info("   Format: YAML frontmatter + Obsidian markdown + #Maestro tag")
